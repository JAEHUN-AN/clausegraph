...
### 5. 남의 문장을 넣어 보니 두 가지가 조용히 틀려 있었다

위 18문항은 **내가 썼다.** 약관을 읽고 만들었으니 약관의 말투가 배어 있다.
그래서 Tier B가 `EXCLUSION`으로 분류한 **실제 분쟁 13건의 쟁점 문장을
그대로** 넣어 봤다.
...
→ [notes/025](notes/025-real-disputes.md)
...
**지연은 캐시를 비싼 호출에 걸어 잡았다.** 청구 1건당 15.8ms → 0.7ms(-96%).
...
전체 설계 근거와 측정 기록은 `notes/`에 025번까지 있다.
