...
**조문 파서는 라벨 대신 같은 문서를 두 번 읽어 채점했다.** 458개에 손으로
라벨을 다는 대신, 같은 별표15의 **PDF 조판본 492쪽**을 레이아웃으로 읽어
평문 경로와 맞댔다. 두 결과가 다르면 둘 중 하나가 틀린 것이라, 라벨 없이
전수 채점이 된다.
...
**개수로는 안 드러나는 결함이 표에서 나왔다.** 표의 행 구분선으로 `┣╋┫`만
보고 있었는데, 이 문서가 더 자주 쓰는 건 굵기만 다른 `┠╂┨`(1,708+1,448회
대 444회)다. 거기서 행을 안 끊으니 아래위 두 행이 합쳐져, **면책 사유 11건이
자기 것이 아닌 보장에 붙어 있었다** — 해외 전용 면책이 국내(급여)에도 달렸다.
사유의 수는 351개로 그대로라 개수를 세는 검사로는 영영 안 보인다.

본문 불일치 81건 중 **66건은 파서가 아니라 평문 원천의 문자 손실**이었다.
XML 평문에는 `?`가 1,088개 있고 그중 줄머리 12개가 항 번호 자리다(`➄`→`?`).
잃은 글자를 PDF에서 베껴 오지 않고 **순서로 되살린 뒤**(④ 다음의 깨진 자리는
⑤다) 조판본으로 검산했다 — **12개 전부 확인**. 추론은 평문 안에서, 검산만
PDF로 한다.

**스캔본으로 오면 얼마나 잃는지도 같은 방법으로 쟀다.** 있는 PDF를 이미지로
구워 글자층을 버리고 OCR로 되읽으면 원래 글자층이 정답이 된다 — CER **9.1%**,
조문 머리글 5/6(300dpi, EasyOCR CPU). 머리글은 살아남지만 글자 100개에 9개가
틀려서, 근거 조항을 인용하는 시스템에는 교정 단계 없이 못 쓴다.
→ [notes/036](notes/036-pdf-layout.md)
