...
TOP_BORDER = "┏"
BOTTOM_BORDER = "┗"
# 행 구분선. 바깥 세로줄에 가로줄이 닿는 자리의 글자들이다.
#
# 오래 `┣╋┫` 세 글자만 봤는데, 이 문서에서 더 자주 쓰이는 건 **굵기가
# 다른 같은 모양** `┠╂┨`이다(각각 1,448 / 180 / 1,708회 대 444회).
# 그 줄에서 행을 끊지 않으면 **서로 다른 보장의 면책이 한 행으로 합쳐진다** —
# 해외여행 실손에서 `해외`의 면책 사유가 `국내(급여)`에도 걸렸다.
# PDF 조판본과 맞대다 드러났다 (notes/036).
#
# `├┼┤`는 넣지 않는다. 가는 세로줄은 셀 **안에 든 표**의 것이다.
