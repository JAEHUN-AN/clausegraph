...
| `triage_definition` | 조문을 찾는 게 아니라 **해석**해야 하는 쟁점일 때. 판정 대신 무엇이 필요한지 |
| `follow_up` | 판정 뒤에 오는 말. "왜요" "뭘 내면 되나요" "얼마예요" |
| `revise_claim` | 새 사실이 나왔을 때. 같은 건을 다시 심사하고 차이를 낸다 |
...
**후속 질문은 앞 답변이 아니라 저장된 판정에서 답한다.** 답변 문장을
이어 쓰면 두 턴 만에 인용이 원본에서 떠내려간다. `follow_up`이 보는 것은
`Claim`과 `Adjudication`이고 둘 다 frozen이다. 그리고 질문에 판정을
바꾸는 값이 섞여 있으면 **답하지 않고** `revise_claim`으로 보낸다 —
낡은 판정을 근거까지 붙여 설명하는 것이 multi-turn에서 가장 나쁜
실패다(notes/032).
